"""
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🤖 PROFESSIONAL TELEGRAM BOT v2.2 - ADMIN MANAGEMENT
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Features: Admin qo'shish/o'chirish, Start rasmi, Kanal sozlash
"""
...
MAIN_ADMIN_ID = os.getenv("ADMIN_ID")
...
class BotConfig:
    ...
    required_channels = []
    user_sessions = {}
    admin_ids = [MAIN_ADMIN_ID]  # Admin IDlari ro'yxati
...
async def admin_add_admin(callback: CallbackQuery, state: FSMContext):
    if not is_main_admin(callback.from_user.id):
        await callback.answer("❌ Faqat bosh admin admin qo'sha oladi!", show_alert=True)
        return
...
async def save_admin_id(message: Message, state: FSMContext):
    ...
    try:
        ...
        if admin_id in BotConfig.admin_ids:
            await message.answer(f"⚠️ Bu ID allaqachon admin!")
            return
        
        BotConfig.admin_ids.append(admin_id)
        await message.answer(
            f"✅ Admin qo'shildi!\n\n"
            f"🆔 ID: {admin_id}\n"
            f"Jami adminlar: {len(BotConfig.admin_ids)} ta"
        )
        await state.clear()
        
    except ValueError:
        await message.answer("❌ Raqamni kiriting!")
...
async def admin_list_admins(callback: CallbackQuery):
    ...
    if not BotConfig.admin_ids:
        await callback.message.answer("👥 Adminlar bo'sh")
        return
    
    buttons = []
    for idx, admin_id in enumerate(BotConfig.admin_ids):
        if admin_id == MAIN_ADMIN_ID:
            buttons.append([InlineKeyboardButton(text=f"👑 {admin_id} (Bosh)", callback_data="noop")])
        else:
            buttons.append([InlineKeyboardButton(text=f"❌ {admin_id}", callback_data=f"delete_admin_{idx}")])
    
    buttons.append([InlineKeyboardButton(text="🔙 Orqaga", callback_data="admin_back")])
    
    admins_text = "👥 Adminlar:\n\n"
    for idx, admin_id in enumerate(BotConfig.admin_ids, 1):
        status = "👑 (Bosh Admin)" if admin_id == MAIN_ADMIN_ID else "(Admin)"
        admins_text += f"{idx}. {admin_id} {status}\n"
    
    await callback.message.answer(admins_text, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
...
async def delete_admin(callback: CallbackQuery):
    ...
    idx = int(callback.data.split("_")[2])
    if 0 <= idx < len(BotConfig.admin_ids):
        deleted_admin = BotConfig.admin_ids.pop(idx)
        await callback.answer(f"✅ {deleted_admin} o'chirildi!")
...
async def admin_view_settings(callback: CallbackQuery):
    ...
    admins_count = len(BotConfig.admin_ids)
...
async def main():
    ...
    print(f"👑 Bosh Admin ID: {MAIN_ADMIN_ID}")
    print(f"👥 Adminlar: {len(BotConfig.admin_ids)} ta")
